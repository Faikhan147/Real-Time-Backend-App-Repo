pipeline {
    ...
    environment {
        ...
        BACKEND_IMAGE_NAME = "faisalkhan35/my-database"
        TAG = "${BUILD_NUMBER}"
        SONAR_PROJECT_KEY = "database"
        SONAR_PROJECT_NAME = "Backend-database"
            ...
            steps {
                script {
                    ...
                    retry(3) {
                        ...
                        sh """
                            helm upgrade --install database-prod ${HELM_CHART_DIR} \
                            --namespace prod \
                            --set ${chartValues} \
                            --set resources.requests.memory=128Mi \
                            --set resources.requests.cpu=100m \
