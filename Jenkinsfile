pipeline {
    ...
    parameters {
        ...
        string(name: 'SONAR_PROJECT_KEY', defaultValue: 'Database', description: 'SonarQube Project Key')
        string(name: 'SONAR_PROJECT_NAME', defaultValue: 'Backend-Database', description: 'SonarQube Project Name')
        string(name: 'REPO_URL', defaultValue: 'https://github.com/Faikhan147/Real-Time-Backend-App-Repo.git', description: 'Git repo URL')
        choice(name: 'ENVIRONMENT', choices: ['qa', 'staging', 'prod'], description: 'Select the environment to deploy')
    }

    environment {
        DOCKER_IMAGE = "faisalkhan35/my-database"
        ...
        HELM_CHART_DIR = "helm/database-chart"
            ...
            steps {
                script {
                    ...
                    sh """
                        ...
                        helm template database-${params.ENVIRONMENT} ${HELM_CHART_DIR} || { echo 'Helm template failed!'; exit 1; }
                    """
                }
            }
        }
        ...
        script {
            ...
            withAWS(credentials: 'aws-credentials', region: 'ap-south-1') {
                ...
                retry(3) {
                    ...
                    sh """
                        helm upgrade --install database-${params.ENVIRONMENT} ${HELM_CHART_DIR} \\
...
stage('Rollback (if needed)') {
    ...
    steps {
        script {
            ...
            def revisionCount = sh(script: "helm history database-${params.ENVIRONMENT} --namespace ${params.ENVIRONMENT} | wc -l", returnStdout: true).trim().toInteger()
            ...
            if (revisionCount >= 3) {
                ...
                sh "helm rollback database-${params.ENVIRONMENT} ${lastRevision} --namespace ${params.ENVIRONMENT}"
            } else {
        ...
        script {
            ...
            retry(3) {
                withEnv(["DATABASE_URL=$DATABASE_URL"]) {
                    sh '''#!/bin/bash
                    echo "🌐 Performing app health check on $DATABASE_URL ..."
                    sleep 10  # wait for service to be ready
                    echo "🔍 DNS resolution test:"
                    nslookup "$DATABASE_URL" || echo "❗ DNS resolution failed."

                    echo "🔍 CURL with verbose output:"
                    curl -v "$DATABASE_URL" || echo "❗ CURL failed."

                    STATUS_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$DATABASE_URL")
                    echo "ℹ️ HTTP Status Code: $STATUS_CODE"

                    if [ "$STATUS_CODE" -ne 200 ]; then
                        echo "❌ App health check failed with status code $STATUS_CODE"
                        exit 1
                    fi
...
stage('Monitor Deployment for Production (Pods + App Health Check)') {
    when {
        expression { return params.ENVIRONMENT == 'prod' }
    }
    steps {
